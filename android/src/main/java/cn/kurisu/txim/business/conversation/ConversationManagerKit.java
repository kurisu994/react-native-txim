...
import cn.kurisu.txim.utils.DateTimeUtil;
...
public class ConversationManagerKit {
    ...
    public void loadConversation(long nextSeq, final ILoadConversationCallback callBack) {
        TIMLog.i(TAG, "loadConversation callBack:" + callBack);
        ...
        V2TIMManager.getConversationManager().getConversationList(nextSeq, GET_CONVERSATION_COUNT, new V2TIMValueCallback<V2TIMConversationResult>() {
            ...
            public void onError(int code, String desc) {
                TIMLog.v(TAG, "loadConversation getConversationList error, code = " + code + ", desc = " + desc);
            }
    ...
    public void onRefreshConversation(List<V2TIMConversation> v2TIMConversationList) {
        TIMLog.i(TAG, "onRefreshConversation conversations:" + v2TIMConversationList);
        if (mProvider == null) {
            return;
        }
        ...
        for (int i = 0; i < v2TIMConversationList.size(); i++) {
            ...
            TIMLog.v(TAG, "refreshConversation v2TIMConversation " + v2TIMConversation.toString());
    ...
    private ConversationInfo TIMConversation2ConversationInfo(final V2TIMConversation conversation) {
        ...
        TIMLog.i(TAG, "TIMConversation2ConversationInfo id:" + conversation.getConversationID()
...
//        MessageInfo messageInfo = MessageInfoUtil.TIMMessage2MessageInfo(message);
        ...
        switch (atInfoType){
            case V2TIMGroupAtInfo.TIM_AT_ME:
                info.setAtInfoText(TIMInitUtil.getAppContext().getString(R.string.ui_at_me));
                break;
            case V2TIMGroupAtInfo.TIM_AT_ALL:
                info.setAtInfoText(TIMInitUtil.getAppContext().getString(R.string.ui_at_all));
                break;
            case V2TIMGroupAtInfo.TIM_AT_ALL_AT_ME:
                info.setAtInfoText(TIMInitUtil.getAppContext().getString(R.string.ui_at_all_me));
                break;
            default:
                info.setAtInfoText("");
                break;
     ...
     * @param conversation
     */
    public void setConversationTop(final ConversationInfo conversation, final TIMCallBack callBack) {
        TIMLog.i(TAG, "setConversationTop" + "|conversation:" + conversation);
        final boolean setTop = !conversation.isTop();
            ...
            V2TIMManager.getConversationManager().pinConversation(conversation.getConversationId(), setTop, new V2TIMCallback() {
                ...
                @Override
                public void onError(int code, String desc) {
                    TIMLog.e(TAG, "setConversationTop code:" + code + "|desc:" + desc);
                    if (callBack != null) {
    ...
    public void setConversationTop(String id, final boolean isTop, final IUIKitCallBack callBack) {
        TIMLog.i(TAG, "setConversationTop id:" + id + "|isTop:" + isTop);
    ...
    public void deleteConversation(int index, ConversationInfo conversation) {
        TIMLog.i(TAG, "deleteConversation index:" + index + "|conversation:" + conversation);
        V2TIMManager.getConversationManager().deleteConversation(conversation.getConversationId(), new V2TIMCallback() {
            @Override
            public void onError(int code, String desc) {
                TIMLog.e(TAG, "deleteConversation error:" + code + ", desc:" + desc);
            }

            @Override
            public void onSuccess() {
                TIMLog.i(TAG, "deleteConversation success");
            }
        });
        if (mProvider != null) {
    ...
    public void clearConversationMessage(int index, ConversationInfo conversation) {
        if (conversation == null || TextUtils.isEmpty(conversation.getConversationId())) {
            TIMLog.e(TAG, "clearConversationMessage error: invalid conversation");
        }

        TIMLog.i(TAG, "clearConversationMessage index:" + index + "|conversation:" + conversation);
        if (conversation.isGroup()) {
            V2TIMManager.getMessageManager().clearGroupHistoryMessage(conversation.getId(), new V2TIMCallback() {
                @Override
                public void onError(int code, String desc) {
                    TIMLog.e(TAG, "clearConversationMessage error:" + code + ", desc:" + desc);
                }

                @Override
                public void onSuccess() {
                    TIMLog.i(TAG, "clearConversationMessage success");
                }
            });
        } else {
    ...
    public void deleteConversation(String id, boolean isGroup) {
        TIMLog.i(TAG, "deleteConversation id:" + id + "|isGroup:" + isGroup);
        ...
        if (!TextUtils.isEmpty(conversationID)) {
            V2TIMManager.getConversationManager().deleteConversation(conversationID, new V2TIMCallback() {
                @Override
                public void onError(int code, String desc) {
                    TIMLog.i(TAG, "deleteConversation error:" + code + ", desc:" + desc);
                }
    ...
    public void updateUnreadTotal(int unreadTotal) {
        TIMLog.i(TAG, "updateUnreadTotal:" + unreadTotal);
    ...
    public void addUnreadWatcher(MessageUnreadWatcher messageUnreadWatcher) {
        TIMLog.i(TAG, "addUnreadWatcher:" + messageUnreadWatcher);
    ...
    public void removeUnreadWatcher(MessageUnreadWatcher messageUnreadWatcher) {
        TIMLog.i(TAG, "removeUnreadWatcher:" + messageUnreadWatcher);
    ...
    public void destroyConversation() {
        TIMLog.i(TAG, "destroyConversation");
        if (mProvider != null) {
            mProvider.clear();
        }
    ...
    public String getGroupConversationAvatar(String conversationId) {
        SharedPreferences sp = TIMInitUtil.getAppContext().getSharedPreferences(
                TIMConfigs.getConfigs().getGeneralConfig().getSDKAppId() + SP_IMAGE, Context.MODE_PRIVATE);
    ...
    public void setGroupConversationAvatar(String conversationId, String url) {
        ...
        if (!success) {
            TIMLog.e(TAG, "setGroupConversationAvatar failed , id : " + conversationId + " , url : " + url);
        }
    }
